...
- **選択素材の配置マップを入力に含める（2026-08-12検証で追加）**: ライター入力に Exclusive placement map（どのimage slotに添付素材が入るか）を必ず渡し、添付素材の見せ方の記述はそのスロット内に限定させる。他のimage枠には「生成する被写体・シーン」を記述させる。検証実験でマップ未入力のままライターが素材を2箇所に配置する散文を書き、末尾の決定論ガードと矛盾して素材が画像内に2回出現した

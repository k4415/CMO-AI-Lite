...
- [ ] **Step 1: compilerの期待動作を表す失敗テストを書く**
...
- [ ] **Step 1: generationとasset付きeditの品質値を表す失敗テストを書く**
...
- [ ] **Step 4: worker 10並列のcharacterization testを追加する**
...
const terminal = await poll(async () => {
  const detail = await getJson(`${baseUrl}/api/project/detail?project=${encodeURIComponent(project)}`);
  return selectBanners(detail.collections, bannerIds);
...
- [ ] **Step 5: checkpoint self-reviewを行う**

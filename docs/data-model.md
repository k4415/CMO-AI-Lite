...
`GET /api/ai-jobs`の応答は表示専用のread modelであり、新しい業務DBではない。状態の正本は次の既存データに置く。

| 処理 | 状態の正本 |
| --- | --- |
| 内部LP解析 | 案件内`data/material-extraction-jobs.json`と`data/research-materials.json` |
| テンプレ解析 | 共通`data/ad-templates.json`の`templateProcessingStatus`、attempt、lease |
| バナー生成・画像修正 | 案件内`data/banner-creatives.json`の`pipelineNodes`、status、lease、`jobRecoveryAudit` |
| 事実抽出・WHO-WHAT・表現レギュレーション抽出 | 同一サーバープロセス内の一時runtime registry |

外部表示statusは`queued` / `running` / `completed` / `completed_with_warnings` / `failed` / `stale`に限定する。`stale`は中断の可能性を示すだけで、GET内でレコードの修正や自動再実行はしない。runtime registryは同期AIの観測補助であり、サーバー再起動で消えてよい。

read modelにはID、種別、表示名、状態、工程、時刻、安全化したエラーだけを含める。LP本文、OCR全文、スクリーンショット配列、copyBrief、promptJson、プロンプト・AI応答本文、APIキー、絶対パスは含めない。
...
出力: WHO-WHAT提案（status: proposed）。新規提案には同じAI応答内で`colorInference`も生成し、`status`（`inferred` / `insufficient`）、4色palette、reason、WHO-WHAT内の根拠`evidence[]`を保存する。既存戦略に未設定の場合は読込時に`insufficient`として扱い、バナー生成中にAI補完しない。

手動追加・編集では`markdown`を1つの本文正本として扱う。UIはMarkdownエディタとプレビューを提供し、`conceptName`は`**戦略コンセプト:**`直後の本文から自動同期する。手動入力を構造化フィールドへ重複保存せず、バナー制作では保存済みMarkdownを選択WHO-WHATとして使用する。
...
- `colorDecision`: 配色決定の監査契約。新規生成はversion 2で、`palette`、フィールド別の`sourceByField` / `reasonByField`、テンプレfallback対象、最終promptの`contractReview`を保持する。旧version 1または未設定も読込可能
...
`main` / `sub` / `accent` / `background`を独立して、`追加指示・修正指示 > 表現レギュレーション > 正式ブランド指定 > 保存済みWHO-WHAT colorInference > templateColorScheme > safe default`の順に解決する。テンプレ文章、shape説明、effect内の具体色は構造の正本にせず、色名とHEXを除去して`colorRole`へ確定paletteを再バインドする。選択ロゴ・商品画像・その他画像は監査・再着色の対象外とし、原本色を維持する。

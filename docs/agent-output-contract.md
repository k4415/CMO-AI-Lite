...
UIから手動追加・編集する場合は、複数の構造化入力欄へ分けず、1つのMarkdown本文を正本として保存する。`conceptName`はMarkdownの`**戦略コンセプト:**`直後にある最初の本文から自動取得する。自動生成戦略は従来どおり構造化フィールドも保持するが、UIでの本文編集は同じMarkdownエディタを使う。

...
2. **candidateIndex=0 を baseline として作る** — テンプレ文面構造・読み順・売れているコピーの型にできるだけ寄せつつ、変数部分だけを選択WHO-WHATの内容へ置き換える。文脈的に不自然な箇所だけ最小限に調整する。
3. **candidateIndex=1 以降を baseline variation として作る** — baseline の主約束・文面骨格・テンプレ適合を保ったまま、フック、根拠の見せ方、CTAトーン、語尾、切り出し方などを変える。同一コピーの焼き直しは禁止。

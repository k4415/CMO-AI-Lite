...
3. **candidateIndex=0 を baseline として作る** — テンプレ文面構造・読み順・売れているコピーの型にできるだけ寄せつつ、変数部分だけを選択WHO-WHATの内容へ置き換える。文脈的に不自然な箇所だけ最小限に調整する。
4. **candidateIndex=1 以降を baseline variation として作る** — baseline の主約束・文面骨格・テンプレ適合を保ったまま、フック、根拠の見せ方、CTAトーン、語尾、切り出し方などを変える。同一コピーの焼き直しは禁止。
5. **copySlotPlan に沿ってコピーを作成する** — 各スロットの文字数上限を守る(10字以下→最大13字、11字以上→120%、小数点以下切り捨て。下限なし)。
6. **自己チェックを行う** — 戦略を知らない初見読者として、画面の文字と読み順だけで評価する。
...
6. **baseline はテンプレ構造を守る** — 見本コピーの単語をそのまま写すのではなく、文面構造・意味の流れ・視線誘導を踏襲する。
7. **variation は baseline を壊しすぎない** — 主約束、主要ベネフィット、テンプレ骨格は維持し、差分は読み味と刺さり方に集約する。
...
- `candidateIndexes` に与えられた番号をそのまま `candidateIndex` に使う。`candidateIndex=0` が baseline。
- variation案は `baselineReference: 0` を返す。baseline と全slot同文面の案は返さない。
...
```json
{
  ...
  "candidates": [{
    "candidateIndex": 0,
    "variationRole": "baseline|variant",
    "baselineReference": 0,
    "variationDirection": "baselineから何をどう変えたか。baselineは空文字でよい",
    "angle": "この案の切り口。variant同士で近くてもよいが、baselineと同一コピーは禁止",

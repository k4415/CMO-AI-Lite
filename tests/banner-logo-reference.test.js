...
test("最終画像プロンプトに添付順とロゴ必須条件を直接含める", () => {
  ...
  assert.match(prompt, /2枚目（02-product-product\.png）: ユーザー選択の商品素材/);
...
test("画像枠0件でも選択ロゴ・商品画像を唯一の構造例外として必須化する", () => {
  ...
  for (const value of [prompt, recoveryPrompt]) {
    ...
    assert.match(value, /各素材を.*1回だけ/);
    assert.match(value, /原画像のまま/);
    assert.match(value, /描き直さない/);
    assert.match(value, /立体化.*パッケージ化.*カード化/);
    assert.match(value, /複製.*反復.*派生/);
...
test("空欄text slotはテンプレの目的や見本から文字を補完しないよう明示する", () => {
  const prompt = buildBannerImagePrompt({
    imageText: "必見",
    promptJson: {
      templateStructureContract: {
        ...
        elementCount: 1,
        typeCounts: { text: 1, image: 0, shape: 0 },
        zones: [{ elements: [{ slotId: "z1e1", type: "text", role: "disclaimer" }] }]
      },
      zones: [{
        name: "Zone 1",
        purpose: "募集終了の限定性を伝える",
        elements: [{ slotId: "z1e1", type: "text", role: "disclaimer", content: "" }]
      }]
    }
  }, []);

  assert.match(prompt, /空欄text slotを推測で補完しない/);
  assert.match(prompt, /content state: EMPTY; render zero glyphs/);
  assert.doesNotMatch(prompt, /exact content: 空欄/);
  assert.doesNotMatch(prompt, /募集終了の限定性を伝える/);
});

test("選択商品素材を写真やパッケージと決めつけず原画像全体を1回だけ使う", () => {
  const prompt = buildBannerImagePrompt({
    imageText: "正式素材で比較",
    promptJson: { zones: [] }
  }, [{ role: "product", ordinal: 1, fileName: "01-product-icon.png", path: "assets/icon.png" }]);

  assert.match(prompt, /ユーザー選択の商品素材/);
  assert.match(prompt, /原画像全体を平面素材として1回だけ/);
  assert.match(prompt, /商品カテゴリや用途を推測しない/);
  assert.match(prompt, /ボトル・箱・容器・立体パッケージへ変換しない/);
  assert.doesNotMatch(prompt, /実際の商品写真/);
});

test("選択商品素材を一つの画像枠だけへ割り当て、他枠での派生利用を禁止する", () => {
  const prompt = buildBannerImagePrompt({
    imageText: "素材を守って比較",
    promptJson: {
      templateStructureContract: { closed: true, zoneCount: 1, elementCount: 3, typeCounts: { text: 0, image: 3, shape: 0 } },
      zones: [{
        name: "visual",
        elements: [
          { type: "image", slotId: "background", role: "illustration", content: "背景" },
          { type: "image", slotId: "person", role: "person", content: "人物" },
          { type: "image", slotId: "foreground", role: "illustration", content: "前景" }
        ]
      }]
    }
  }, [{ role: "product", ordinal: 1, fileName: "01-product-icon.png", path: "assets/icon.png" }]);

  assert.match(prompt, /placement target: image slot foreground only/);
  assert.match(prompt, /background.*do not use or imitate any selected attached asset/);
  assert.match(prompt, /person.*do not use or imitate any selected attached asset/);
  assert.match(prompt, /foreground.*use attached image 1 only/);
});

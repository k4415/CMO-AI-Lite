...
test("決定論的promptも正本のWHO-WHAT markdownと案別variationを画像プロンプトへ残す", async () => {
  ...
  assert.match(proposal.promptText, /人物を追加しない/);
  assert.equal(proposal.promptJson.colorScheme.background, "#FFFFFF");
...
test("決定論的compilerは具体色を決めず、色中立のshape構造だけを返す", () => {
  const templateZones = [{
    position: "top",
  ...
  assert.deepEqual(seed.promptJson.colorScheme, {});
  assert.match(seed.promptJson.zones[0].elements[0].content, /戻る矢印/);
  assert.doesNotMatch(seed.promptJson.zones[0].elements[0].content, /青/);
  assert.doesNotMatch(seed.promptJson.zones[0].elements[0].content, /ゴールド/);
  const neutralTemplateZones = buildColorNeutralTemplateZones(templateZones, {});
  const enforced = enforceTemplateStructure({
    templateZones: neutralTemplateZones,
    generatedZones: seed.promptJson.zones
  });
  assert.match(enforced.zones[0].elements[0].content, /戻る矢印/);
  assert.doesNotMatch(enforced.zones[0].elements[0].content, /青/);
...
test("4つの配色シナリオを追加AIなしで最終promptまで統合する", async () => {
  const templates = JSON.parse(await fs.readFile(new URL("../data/ad-templates.json", import.meta.url), "utf8"));
  const inferredPalette = { main: "#16324F", sub: "#E2E8F0", accent: "#F28C28", background: "#F7FAFC" };
  const inferredStrategy = {
    id: "strategy-color",
    decisionCriteria: "信頼できること",
    markdown: "WHO: 広告運用担当者\nWHAT: 信頼できる制作判断",
    colorInference: {
      status: "inferred",
      palette: inferredPalette,
      reason: "信頼できる判断と行動喚起を両立する",
  ...
  const scenarios = [
    {
      name: "user",
      templateId: "tpl_default_026",
      banner: { additionalInstruction: "背景は白、アクセントカラーは青" },
      strategy: { ...inferredStrategy, colorInference: { status: "insufficient", palette: {}, reason: "根拠不足", evidence: [] } },
      expected: { background: "user_instruction", accent: "user_instruction" }
    },
    {
      name: "regulation",
      templateId: "tpl_default_032",
      expressionRules: [{ ruleType: "color", description: "メインカラーは#003366、アクセントカラーは#FF6600" }],
      strategy: inferredStrategy,
      expected: { main: "regulation", sub: "who_what_inference", accent: "regulation", background: "who_what_inference" }
    },
    {
      name: "who_what",
      templateId: "tpl_default_071",
      strategy: inferredStrategy,
      expectedSource: "who_what_inference"
    },
    {
      name: "template",
      templateId: "tpl_default_026",
  ...
  for (const scenario of scenarios) {
    ...
    const copyBrief = {
      version: 3,
      strategyId: scenario.strategy.id,
      appealAxis: scenario.name,
      whyItStops: "配色契約の統合検証",
    ...
    const proposal = await generateBannerCreativeProposal({
      ...
      template,
      expressionRules: scenario.expressionRules || [],
      copyBrief,
      jsonGenerator: async () => { throw new Error("closed template must not call model"); }
    });

    assert.equal(proposal.promptGenerationAudit.modelDesignCalls, 0, scenario.name);
    assert.deepEqual(proposal.promptGenerationAudit.httpAttempts, [], scenario.name);
    assert.deepEqual(paletteOnly(proposal.promptJson.colorScheme), proposal.colorDecision.palette, scenario.name);
    assert.equal(proposal.colorDecision.contractReview.status, "passed", scenario.name);
    assert.equal(proposal.promptJson.zones.length, template.templateZones.length, scenario.name);
    assert.equal(countElements(proposal.promptJson.zones), countElements(template.templateZones), scenario.name);
    if (scenario.expectedSource) assert.equal(proposal.colorDecision.source, scenario.expectedSource, scenario.name);
    for (const [field, source] of Object.entries(scenario.expected || {})) {
      assert.equal(proposal.colorDecision.sourceByField[field], source, `${scenario.name}/${field}`);
    }
    for (const color of Object.values(template.templateColorScheme || {}).filter((value) => /^#[0-9a-f]{6}$/i.test(String(value)))) {
      if (!Object.values(proposal.colorDecision.palette).includes(String(color).toUpperCase())) {
        assert.equal(JSON.stringify(proposal.promptJson).toUpperCase().includes(String(color).toUpperCase()), false, `${scenario.name}/${color}`);
      }
    }
  }
});
...
function paletteOnly(value = {}) {
  return { main: value.main, sub: value.sub, accent: value.accent, background: value.background };
}

function countElements(zones) {
  return (Array.isArray(zones) ? zones : []).reduce((count, zone) => count + (zone.elements || []).length, 0);
}

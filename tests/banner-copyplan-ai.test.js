...
const okResponse = {
  ...
  candidates: [
    { candidateIndex: 0, angle: "時短", slotTexts: [{ slotId: "s1", text: "毎朝10分短縮" }], semanticGroupReadout: [], appealAxis: "時短", whyItStops: "毎朝10分という具体的な変化が一読で伝わるため", selfCheck: { blindReadability: "pass", system1Impact: "pass", coherence: "pass", strategyFit: "pass", issues: [] } },
    { candidateIndex: 1, angle: "初回無料", slotTexts: [{ slotId: "s1", text: "初回無料で試せる" }], semanticGroupReadout: [], appealAxis: "オファー", whyItStops: "初回無料という行動障壁の低さが即座に伝わるため", selfCheck: { blindReadability: "pass", system1Impact: "warn", coherence: "pass", strategyFit: "pass", issues: ["数字が弱い"] } }
  ]
};

test("1コールでN案生成し、selfCheck warnはstatus=warningで通す", async () => {
  const calls = [];
...
test("angleが全案で重複した応答は不正扱いでバッチリトライされる", async () => {
  const duplicated = { ...okResponse, candidates: okResponse.candidates.map((c) => ({ ...c, angle: "時短" })) };
  let call = 0;
